...
* Создание Deployment приложения, состоящего из контейнеров busybox и multitool.

![Создание Deployment приложения](img/01.png)  

* Настройка busybox на запись данных каждые 5 секунд в некий файл в общей директории.

![Проверка записи данных в файл](img/02.png)  

* Обеспечение возможности чтения файла контейнером multitool.

![Чтение данных данных](img/03.png)  
![Чтение данных данных](img/04.png)  

Описание пода с контейнерами:

![Описание пода с контейнерами](img/05.png)  

## Задание 2. PV, PVC

* Создание Deployment приложения, состоящего из контейнеров busybox и multitool, использующего созданный ранее PVC
* Создание PV и PVC для подключения папки на локальной ноде, которая будет использована в поде.

![Создание Deployment приложения](img/06.png)  

* Демонстрация, что контейнер multitool может читать данные из файла в смонтированной директории, в который busybox записывает данные каждые 5 секунд.

![Демонстрация чтения данных](img/07.png)  

* Удаление Deployment и PVC. Демонстрация того, что после этого произошло с PV. Пояснение, почему.

![Удаление Deployment и PVC](img/08.png)  

`PV` перешёл в статус `Released`, потому что у него задана политика `persistentVolumeReclaimPolicy: Retain`. При удалении `PVC` `Kubernetes` не удаляет ни сам `PV`, ни данные на диске - он лишь отвязывает том от исчезнувшей заявки и помечает его как "освобождённый". Это защита от случайной потери данных: чтобы переиспользовать или удалить том, это требуется сделать вручную.

* Демонстрация, что файл сохранился на локальном диске ноды. Удалить PV. Демонстрация, что произошло с файлом после удаления PV. Пояснение, почему.

![Демонстрация сохранения файла локально](img/09.png)  
![Демонстрация сохранения файла локально](img/10.png)  

## Задание 3. StorageClas

* Создание Deployment приложения, состоящего из контейнеров busybox и multitool, использующего созданный ранее PVC.
* Создание SC и PVC для подключения папки на локальной ноде, которая будет использована в поде.

![Демонстрация создания](img/11.png)  

* Демонстрация, что контейнер multitool может читать данные из файла в смонтированной директории, в который busybox записывает данные каждые 5 секунд.

![Демонстрация чтения данных](img/12.png)  
